...
- `components/navigation/SiteFooter.tsx` uses the active route to omit footer navigation and its divider on the home orrery page, right-aligning the copyright there while preserving the full footer on other routes.
- `app/globals.css` contains the global tokens, layout, responsive rules, visual treatment and animation styles; `SiteFooter.module.css` provides the small route-specific footer adjustment. There is no utility framework.

### Home and orbital interaction

- `components/orbital/OrbitalSystem.tsx` is the main client-side visual system. It draws artist orbits and a central Works node on a canvas, calculates hit targets, slows an artist orbit on hover, optionally starts an artist preview, opens an artist dialog after a three-second hover, and navigates to an artist detail page or the Works archive on click. The dialog is rendered through a portal above the page, maintains a responsive circular shape, fades in without entrance movement, and is surrounded by three fixed circular orbit paths sized 1×, 1.2× and 1.4× from inner to outer. The `--popup-orbit-radius` CSS custom property controls the popup diameter and the shared orbit base, so the first orbit always traces the popup edge while resizing keeps every planet track aligned. Their planet markers rotate slowly in a layer above the dialog surface; closing fades the complete overlay out. Its backdrop is a plain 95%-black overlay rather than a live blur, avoiding the cost of compositing the entire page behind the popup.
